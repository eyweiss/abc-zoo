...
const Btn = ({ onClick, bg, color, border, children, style = {} }) => (
  <button onClick={onClick} style={{
    ...
    borderRadius: "3vw", cursor: "pointer",
...
function TraceCanvas({ letter, color, onBack }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"2vw", width:"100%", height:"100%" }}>
...
export default function ABCZoo() {
  ...
  useEffect(() => {
    ...
    s.textContent = `
      ...
      *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
      ...
      button:active { transform:translateY(0.6vw) !important; }
      @keyframes slideR { from{opacity:0;transform:translateX(6vw)} to{opacity:1;transform:translateX(0)} }
      @keyframes slideL { from{opacity:0;transform:translateX(-6vw)} to{opacity:1;transform:translateX(0)} }
      @keyframes popIn { from{opacity:0;transform:scale(0.5)} to{opacity:1;transform:scale(1)} }
      @keyframes bk { 0%,100%{transform:scale(1)} 50%{transform:scale(1.25)} }
      @keyframes floatUp { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-1.5vw)} }
      @keyframes shake { 0%,100%{transform:translateX(0)} 25%{transform:translateX(-3vw)} 75%{transform:translateX(3vw)} }
      @keyframes confUp { 0%{transform:translateY(0) rotate(0);opacity:1} 100%{transform:translateY(-50vw) rotate(720deg);opacity:0} }
      .sr { animation:slideR 0.25s ease; }
      .sl { animation:slideL 0.25s ease; }
      .pi { animation:popIn 0.35s cubic-bezier(0.34,1.56,0.64,1); }
      .bk { animation:bk 0.4s ease; }
      .fl { animation:floatUp 3s ease-in-out infinite; }
      .sh { animation:shake 0.35s ease; }
      .conf { position:fixed; font-size:7vw; animation:confUp 1.5s ease-out forwards; z-index:9999; pointer-events:none; bottom:15%; }
  ...
  // Full-screen, NO overflow — everything must fit
  ...
  if (screen === "learn") {
    ...
    return (
      <Wrap bg={`linear-gradient(160deg,${cur.color}33 0%,#eef2ff 60%)`}>

        {/* Row 1: Header — fixed height */}
        <div style={{ flexShrink:0, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"2.5vw 4vw", height:"14vw" }}>
          <Btn onClick={()=>{ setTracing(false); setScreen("home"); }} bg="rgba(255,255,255,0.92)" color="#444" border="1px solid #ddd" style={{ fontSize:"4vw", padding:"2vw 4vw" }}>← Home</Btn>
          <div style={{ background:"rgba(255,255,255,0.92)", borderRadius:"8vw", padding:"2vw 5vw", display:"flex", gap:"2vw", alignItems:"center", border:"1px solid #ddd", fontSize:"4.5vw", fontWeight:800 }}>⭐ {learned.size}/26</div>
          <Btn onClick={startQuiz} bg={cur.color} color="#fff" style={{ fontSize:"4vw", padding:"2vw 4vw" }}>🎮 Quiz</Btn>
        </div>

        {/* Row 2: ABC strip — fixed height */}
        <div style={{ flexShrink:0, overflowX:"auto", padding:"0 4vw 2vw", height:"14vw", WebkitOverflowScrolling:"touch" }}>
          <div style={{ display:"flex", gap:"2vw", height:"100%", alignItems:"center" }}>
            {ALPHABET.map((a,i)=>(
              <button key={a.letter} onClick={()=>goToLetter(i)} style={{
                width:"10vw", height:"10vw", borderRadius:"2.5vw", flexShrink:0,
                background:i===idx?cur.color:learned.has(a.letter)?"#d4edda":"#fff",
                color:i===idx?"#fff":"#444", fontWeight:900, fontSize:"4vw",
        ...
        {/* Row 3: Main card — takes remaining space */}
        <div style={{ flex:1, display:"flex", flexDirection:"column", padding:"0 4vw 2vw", minHeight:0, gap:"2vw" }}>
          <div key={cardKey} className={direction>=0?"sr":"sl"}
            style={{ flex:1, background:"#fff", borderRadius:"5vw", boxShadow:"0 2vw 8vw rgba(0,0,0,0.13)", padding:"4vw", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"space-evenly", minHeight:0, overflow:"hidden" }}>

            {!tracing ? (<>
              {/* Big letters */}
              <div style={{ display:"flex", alignItems:"center", gap:"4vw" }}>
                <span style={{ fontSize:"22vw", fontWeight:900, color:cur.color, lineHeight:1 }}>{cur.letter}</span>
                <span style={{ fontSize:"17vw", fontWeight:900, color:`${cur.color}88`, lineHeight:1 }}>{cur.letter.toLowerCase()}</span>
              </div>

              {/* Emoji + name */}
              <div key={`e${idx}`} className="pi" style={{ fontSize:"20vw", lineHeight:1 }}>{cur.emoji}</div>
              <div style={{ fontSize:"6.5vw", fontWeight:800, color:"#333" }}>{cur.animal}</div>

              {/* Buttons */}
              <div style={{ display:"flex", gap:"3vw", width:"100%" }}>
                <Btn onClick={()=>speak(`The letter ${cur.letter}! ${cur.letter} is for ${cur.animal}!`)}
                  bg={cur.color} color="#fff" style={{ flex:1, fontSize:"4.5vw", padding:"3.5vw 2vw" }}>🔊 Say it!</Btn>
                <Btn onClick={()=>setTracing(true)}
                  bg="#f4f4f8" color="#555" border="2px solid #e0e0e0" style={{ flex:1, fontSize:"4.5vw", padding:"3.5vw 2vw" }}>✏️ Trace!</Btn>
              </div>

              {/* Learned */}
              <button onClick={toggleLearned} className={isLearned?"bk":""}
                style={{ fontFamily:"'Nunito',sans-serif", fontWeight:800, fontSize:"4.5vw", background:isLearned?"#2ED573":"#fafafa", color:isLearned?"#fff":"#bbb", border:isLearned?"none":"2.5px dashed #ddd", borderRadius:"10vw", padding:"3.5vw 0", cursor:"pointer", width:"100%", transition:"all 0.2s" }}>
          ...
          {/* Nav arrows */}
          <div style={{ flexShrink:0, display:"flex", gap:"5vw", alignItems:"center", justifyContent:"center", height:"12vw" }}>
            <Btn onClick={()=>nav(-1)} bg="#fff" color="#444" border="2px solid #ddd" style={{ fontSize:"6vw", padding:"2.5vw 5vw" }}>←</Btn>
            <span style={{ fontWeight:800, color:"#777", fontSize:"4.5vw", minWidth:"12vw", textAlign:"center" }}>{idx+1} / 26</span>
  ...
  if (screen === "quiz" && qIdx < quiz.length) {
    ...
    return (
      <Wrap bg="linear-gradient(155deg,#f093fb 0%,#f5576c 100%)">
        ...
        {/* Header */}
        <div style={{ flexShrink:0, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"3vw 4vw", height:"14vw" }}>
          <Btn onClick={()=>setScreen("home")} bg="rgba(255,255,255,0.2)" color="#fff" border="1px solid rgba(255,255,255,0.4)" style={{ fontSize:"4vw", padding:"2vw 4vw" }}>✕ Exit</Btn>
        ...
        {/* Progress */}
        <div style={{ flexShrink:0, background:"rgba(255,255,255,0.25)", borderRadius:"3vw", height:"3vw", margin:"0 4vw 3vw" }}>
          <div style={{ background:"#fff", borderRadius:"3vw", height:"100%", width:`${pct}%`, transition:"width 0.5s" }} />
        </div>

        {/* Card */}
        <div style={{ flex:1, display:"flex", flexDirection:"column", padding:"0 4vw 4vw", minHeight:0 }}>
          <div className="pi" key={qIdx} style={{ flex:1, background:"#fff", borderRadius:"5vw", boxShadow:"0 2vw 10vw rgba(0,0,0,0.2)", padding:"4vw", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"space-evenly", minHeight:0, overflow:"hidden" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3vw", width:"100%", flex:1, maxHeight:"50vw" }}>
              {q.options.map((opt)=>{
                ...
                return (
                  <button key={opt.letter} onClick={()=>handleAnswer(opt)}
                    ...
                    style={{ fontFamily:"'Nunito',sans-serif", background:bg, color:tc, border, borderRadius:"4vw", padding:"2vw", cursor:selected?"default":"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"1.5vw", boxShadow:"0 1.5vw 0 rgba(0,0,0,0.08)", transition:"background 0.2s" }}>
                    {isL2A
                      ? <><span style={{ fontSize:"10vw" }}>{opt.emoji}</span><span style={{ fontSize:"3vw", fontWeight:800 }}>{opt.animal}</span></>
                      : <span style={{ fontSize:"14vw", fontWeight:900 }}>{opt.letter}</span>
                    }
                  </button>
                );
              })}
